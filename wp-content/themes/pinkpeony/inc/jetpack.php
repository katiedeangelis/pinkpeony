 ...
 */
function pinkpeony_jetpack_setup() {
	...
	add_theme_support( 'infinite-scroll', array(
		'container' => $container,
		'render'    => 'pinkpeony_infinite_scroll_render',
		'footer'    => 'page',
	) );

	add_theme_support( 'featured-content', array(
		'filter'     => 'pinkpeony_get_featured_posts',
		'max_posts'  => 10,
...
if ( function_exists( 'jetpack_is_mobile' ) && class_exists( 'Jetpack_User_Agent_Info' ) ) {
	...
	add_filter( 'infinite_scroll_has_footer_widgets', 'pinkpeony_has_footer_widgets' );
}

/**
 * Custom render function for Infinite Scroll.
 */
function pinkpeony_infinite_scroll_render() {
	if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_taxonomy() || is_product_category() || is_product_tag() ) ) {
		pinkpeony_woocommerce_product_columns_wrapper();
	...
	if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_taxonomy() || is_product_category() || is_product_tag() ) ) {
		...
		pinkpeony_woocommerce_product_columns_wrapper_close();
	}
}

function pinkpeony_get_featured_posts() {
	return apply_filters( 'pinkpeony_get_featured_posts', array() );
}

function pinkpeony_has_featured_posts( $minimum = 1 ) {
	if ( is_paged() )
		return false;

	$minimum = absint( $minimum );
	$featured_posts = apply_filters( 'pinkpeony_get_featured_posts', array() );
 ...
 */
function pinkpeony_author_bio_avatar_size() {
	return 90;
}
add_filter( 'jetpack_author_bio_avatar_size', 'pinkpeony_author_bio_avatar_size' );

function pinkpeony_social_menu() {
 ...
 */
function pinkpeony_has_post_thumbnail( $post = null ) {
 ...
 */
function pinkpeony_custom_gallery_content_width() {
    return 1086;
}
add_filter( 'gallery_widget_content_width', 'pinkpeony_custom_gallery_content_width' );
